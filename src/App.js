...
function App() {
  ...
  // Function to generate the URL for current weather
  const getCurrentWeatherUrl = (location) =>
    `https://api.openweathermap.org/data/2.5/weather?q=${location}&units=imperial&appid=f97ef50d255f411a64f802719315b8aa`;
  
  // Function to generate the URL for forecast
  const getForecastUrl = (location) =>
    `https://api.openweathermap.org/data/2.5/forecast?q=${location}&units=imperial&appid=f97ef50d255f411a64f802719315b8aa`;
  ...
  const searchLocation = async (event) => {
    if (event.key === 'Enter') {
      try {
        ...
        const currentWeatherResponse = await axios.get(getCurrentWeatherUrl(location));
        setData(currentWeatherResponse.data);
        
        // Fetch forecast data
        const forecastResponse = await axios.get(getForecastUrl(location));
